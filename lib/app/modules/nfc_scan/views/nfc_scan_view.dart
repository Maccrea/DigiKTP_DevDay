...
class NfcScanView extends GetView<NfcScanController> {
  ...
  String _getStepTitle(ScanStep step) {
    switch (step) {
      ...
      case ScanStep.sendOtp: return 'Pengiriman Kode OTP';
      case ScanStep.inputOtp: return 'Verifikasi Kode OTP';
      case ScanStep.confirmation: return 'Konfirmasi Pengajuan';
      case ScanStep.success: return 'Registrasi Berhasil';
    }
  }
  ...
  Widget build(BuildContext context) {
    return PopScope(
      ...
      child: Scaffold(
        ...
        body: Column(
          children: [
            // HEADER FLOW INDICATOR & JUDUL STEP (Dengan SafeArea built-in)
            Obx(() {
              if (controller.currentStep.value == ScanStep.success) {
                return const SizedBox.shrink(); // Sembunyikan header jika sukses
              }
              return _buildPremiumFlowHeader(controller.currentStep.value);
            }),

            // KONTEN UTAMA SESUAI STEP
            Expanded(
              child: Obx(() {
  ...
  Widget _buildPremiumFlowHeader(ScanStep currentStep) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(
            color: const Color(0xFF0F172A).withOpacity(0.04), // Soft shadow elegan
            blurRadius: 20,
                  ...
                  if (i.isEven) {
                    ...
                    return AnimatedContainer(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: isActive
                            ? Border.all(color: const Color(0xFFDBEAFE), width: 4)
                            : Border.all(
                                color: isCompleted
                                    ? const Color(0xFF10B981)
