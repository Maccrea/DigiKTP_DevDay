...
class NfcScanController extends GetxController {
  ...
  void bypassScan() {
    scannedUid.value = '0E:27:12:10:40:6F:33'; 
  ...
  Future<void> requestOtpApi(int selectedOptionIndex) async {
    try {
      ...
      final nfcUid = scannedUid.value.trim();
      if (nfcUid.isEmpty) {
  ...
  Future<void> verifyOtpApi() async {
    ...
    if (enteredOtp.length < 6) {
      Get.snackbar('Peringatan', 'Masukkan 6 digit kode OTP secara lengkap.', snackPosition: SnackPosition.TOP);
      return;
    }
    ...
    try {
      ...
      final responseData = await _apiProvider.verifyOtp(
        nfcUid: _otpNfcUid ?? scannedUid.value.trim(),
        otpCode: enteredOtp,
        idPetugas: 'PTG-001',          
        idInstansi: 'INST-BPJS-01',    
